        ...
        with cols[i % 3]:
            with st.container():
                ...
                ">
                    ...
                    <p style="margin: 4px 0; font-size: 12px;">👤 {proyecto.asignado_a.nombre}</p>
                    <p style="margin: 4px 0; font-size: 12px;">🏢 {proyecto.cliente.nombre}</p>
                    <p style="margin: 4px 0; font-size: 12px; color: #666;">💰 {valor_formateado} <small>({proyecto.moneda})</small></p>
                    <p style="margin: 4px 0; font-size: 12px;">{deadline_html}</p>
                ...
                with col5:
                    ...
                    if historial:
                        ...
                        for evento in historial:
                            fecha = evento[0].strftime('%d/%m %H:%M')
                            st.markdown(f"_{fecha}:_ {evento[1]}")
                    else:
                        st.info("Sin historial reciente")

# ==============================
# Footer (mantenido igual)
# ==============================
st.markdown("---")
st.markdown("📊 **Dashboard de Oportunidades** - Desarrollado con Streamlit y SQLAlchemy ORM")
